...
Detail levels: `short` (~60 tokens), `medium` (~300), `full` (~900), `exhaustive` (~1,800). Tokens target the bounded-response contract (NFR-PERF-03).
...
Opt-in via `clarion.yaml:serve.auth: token` (default: `none`). Required for CI / Wardline use cases where loopback is not a security boundary.

- Token format: 32 random bytes base64-encoded (43 chars), prefixed `clrn_` for grep-ability in logs
- Generated by `clarion serve auth init`
- Storage (in order of preference): OS keychain via `keyring` crate; `~/.config/clarion/token` with mode `0600` fallback (emits `CLA-INFRA-TOKEN-STORAGE-DEGRADED`)
- Wire: `Authorization: Bearer clrn_<43chars>`; constant-time server comparison
- Rotation: `clarion serve auth rotate` with 24-hour grace window
- Scoping: v0.1 has one project-wide read token; per-endpoint scoping is v0.2

Wardline in CI picks up the token via `CLARION_TOKEN` env (preferred) or `~/.config/clarion/token` bind-mount. `clarion check-auth --from wardline` returns exit 0 if authentication succeeds — a pre-flight CI check.
...
| Token leak via HTTP API | High | `clarion serve` on shared dev host | Token auth; OS keychain storage |
...
| Plugin subprocess compromise | Medium | Malicious third-party plugin reads source | Trusted-source-only posture (v0.1); plugin hash-pinning v0.2 (NG-16) |
...
- **Plugin sandbox** (seccomp/landlock). Plugins run with the user's full privilege. Trusted-source-only posture documented.
...
3. **Filigree `/api/v1/observations`**: `HEAD` request. Absent → MCP-client fallback.
...
| Filigree present; `/api/v1/observations` absent | MCP-client transport: Clarion spawns `filigree mcp` | `CLA-INFRA-FILIGREE-OBS-VIA-MCP` |
...
**Filigree side** (ADRs 014, 016, 017 in detailed-design):
- `registry_backend` config flag + pluggable `RegistryProtocol` (the schema-surgery)
- `POST /api/v1/observations` HTTP endpoint
...
|---|---|---|---|---|
| ADR-001 | Rust for the core | Directive | P0 | Primary author directive. Single-binary ship, mature ecosystem (axum/rusqlite/tokio), plugin interop via subprocess. Not subject to alternatives analysis. |
| ADR-002 | Plugin transport: Content-Length framed JSON-RPC 2.0 subprocess | To author | P0 | Binary-safe framing, resumability after crash, alignment with LSP patterns. Alternatives: newline-delimited JSON (unsafe for content with embedded newlines), Wasm (too early for plugin authoring ergonomics), embedded Python (couples core to Python runtime). |
| ADR-003 | Entity ID scheme: symbolic canonical-name; file path as property; EntityAlias v0.2 | To author | P0 | Cross-tool identity must survive file moves. Path-embedded IDs silently detach every reference; symbolic IDs survive 80% case (file move without rename). Rename tracking via EntityAlias deferred; manual `--repair-aliases` workaround in v0.1. |
| ADR-004 | Finding-exchange format: Filigree-native intake; `metadata.clarion.*` nesting | To author | P0 | Filigree's `POST /api/v1/scan-results` is production path; SARIF requires either translation or Filigree-side work. Nesting convention under `metadata` dict (verified verbatim preservation) avoids silent drops of extension fields. |
...
| ADR-006 | Clustering algorithm: Leiden (with Louvain fallback) on imports + calls subgraph | To author | P1 | Leiden produces better modularity than Louvain on heterogeneous graphs at acceptable cost. Clustering quality drives Phase-6 subsystem synthesis (Opus calls — expensive), so algorithm choice is load-bearing. Louvain as fallback if Leiden's edge-reference implementation proves unstable. |
| ADR-007 | Summary cache key design: `(entity_id, content_hash, prompt_template_id, model_tier, guidance_fingerprint)` + TTL backstop + churn-eager invalidation | To author | P1 | Full key captures all syntactic staleness paths; TTL backstops semantic staleness the key alone doesn't see. Churn-eager invalidation makes stale-guidance pressure visible via cost, not just review cadence. |
...
| ADR-011 | Writer-actor concurrency model (vs shadow-DB swap) | To author | P1 | Single writer actor + per-N-files transactions is the default; shadow-DB via `--shadow-db` flag available for zero-stale-read scenarios. Writer-actor handles both analyze and serve without contention. |
| ADR-012 | Token auth in v0.1 (opt-in default; OS keychain preferred) | To author | P1 | Loopback is not a security boundary on modern dev hosts. Designing token auth in v0.1 (even if default is `none`) avoids retrofitting after a leak. |
| ADR-013 | Pre-ingest secret scanner (detect-secrets) with LLM-dispatch block | To author | P1 | First real user running analyze on a repo with committed `.env` leaks to Anthropic silently. Pre-ingest redaction is a hard dependency for v0.1. |
| ADR-014 | Filigree `registry_backend` flag + pluggable `RegistryProtocol` — schema surgery, not config flip | To author | P0 | Four NOT-NULL foreign keys on `file_records(id)` + three auto-create paths require a real interface, not a flag. Clarion's shadow-registry fallback preserves v0.1 shipability when Filigree hasn't landed the surgery. |
| ADR-015 | Wardline→Filigree emission ownership: Clarion-side SARIF translator (v0.1), native Wardline POST (v0.2) | To author | P0 | Wardline has no HTTP client today; adding one is a significant refactor not on the v0.1 timeline. Clarion-side translator ships independently; translator stays permanent (Semgrep / CodeQL / etc. remain SARIF). |
| ADR-016 | Observation transport: Filigree adds `POST /api/v1/observations`; Clarion MCP-client workaround | To author | P0 | Clarion is a Rust binary; MCP-client transport to Filigree adds protocol-compatibility weight. HTTP is the natural transport; Filigree's observation API is MCP-only today. Workaround (spawn `filigree mcp`) preserves v0.1 shipability until Filigree ships the endpoint. |
| ADR-017 | Severity mapping + rule-ID round-trip + dedup via `mark_unseen=true` | To author | P0 | Clarion internal severity (`INFO/WARN/ERROR/CRITICAL`) maps to Filigree wire (`critical/high/medium/low/info`). Round-trip preserves internal via `metadata.clarion.internal_severity`. `mark_unseen=true` is the v0.1 dedup workaround for entities moving within files. |
| ADR-018 | Identity reconciliation: Clarion maintains translation layer; Wardline owns its qualnames; direct REGISTRY import with version pinning | To author | P0 | Three identity schemes exist (Clarion EntityId, Wardline qualname, exception-register location). Clarion is the translator (Principle 3). Forcing Wardline to adopt Clarion's scheme would be centralisation drift. |
...
Terms used in this document are defined in the detailed-design's Appendix B. Pointer-only here to avoid duplication — the detailed-design's glossary is the canonical vocabulary for the three-layer docset.
